...
def test_chat_message_initialization():
    cm = ChatMessage(Role.USER, {DisplayType.DISPLAY: "Hello"})
    assert cm.role == Role.USER
...
def test_chat_message_special_methods():
    cm = ChatMessage(Role.USER, {DisplayType.DISPLAY: "Hello", DisplayType.SUMMARY: "Summary"})
...
def test_history_methods():
    history = History()
    cm1 = ChatMessage(Role.USER, {DisplayType.DISPLAY: "Hello"})
    cm2 = ChatMessage(Role.ASSISTANT, {DisplayType.DISPLAY: "Hi"})
    cm3 = ChatMessage(Role.SYSTEM, {DisplayType.DISPLAY: "Info"})
    ...
    last_message, idx = history.get_last_message_of_type(Role.USER)
    assert last_message.display == "Hello"
    assert idx == 0

    assert history.last_system_message.display == "Info"
    assert history.last_role == Role.SYSTEM
    assert history.turn_state == TurnState.AWAITING_ACTION

    history.pop()
    assert len(history) == 2

    history2 = History([ChatMessage(Role.USER, {DisplayType.DISPLAY: "New Message"})])
...
def test_history_filter_methods():
    ...
    cm1 = ChatMessage(Role.USER, {DisplayType.DISPLAY: "Hello"})
    cm2 = ChatMessage(Role.ASSISTANT, {DisplayType.DISPLAY: "Hi", DisplayType.CONTEXT: "Greeting context"})
    cm3 = ChatMessage(Role.USER, {DisplayType.DISPLAY: "Info", DisplayType.CONTEXT: "Info context"})
...
def chat_history():
    ...
    messages = [
        ChatMessage(Role.USER, {DisplayType.DISPLAY: "Hello 1"}),
        ChatMessage(Role.ASSISTANT, {DisplayType.DISPLAY: "Hi 1", DisplayType.CONTEXT: "Context 1"}),
        ChatMessage(Role.USER, {DisplayType.DISPLAY: "Hello 2"}),
        ChatMessage(Role.ASSISTANT, {DisplayType.DISPLAY: "Hi 2", DisplayType.CONTEXT: "Context 2"}),
    ]
