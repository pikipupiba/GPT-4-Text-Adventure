...
class Game:
    ...
    def submit(
        ...
        complete_system_message = SystemMessage.inject_schemas(
            system_message, Game._num_turns(game_name)
        )
        ...
        if (Game._num_turns(game_name) == 0) or (
            (Game._last_raw(game_name)[1] is not None)
            and (len(Game._last_raw(game_name)[1]) > 0)
        ):
    ...
    def stream_prediction(game_name: str, audio_speed: int = 125):
        try:
            ...
            while new_day and new_day_counter < 3:
                ...
                schema_delimiter = (  # regex pattern to find schema delimiters
                    r"\.\.\s*[A-Z]+\s*\.\."
                )
                schema_name = None
                item_index = None
                ...
                for chunk in LLM.predict(
                    model, system_message, raw_history, Game._(game_name).llm_model
                ):
                    ...
                    if not schema_name:
                        ...
                        opening_match = re.search(
                            schema_delimiter, Game._last_display(game_name)[1]
                        )
                        if opening_match:
                            schema_name = opening_match.group(0).strip(" .")
                            Game._last_display(game_name)[1] = Game._last_display(
                                game_name
                            )[1][: opening_match.start()]
                    ...
                    if schema_name == "DAY":
                        ...
                        closing_match = re.search(
                            schema_delimiter, Game._last_turn(game_name).stats["DAY"]
                        )
                        if closing_match:
                            schema_name = None
                            Game._last_turn(game_name).stats["DAY"] = Game._last_turn(
                                game_name
                            ).stats["DAY"][: closing_match.start()]

                            # Extract the number from the day string
                            numbers_in_day = re.findall(
                                r"\d+", Game._last_turn(game_name).stats["DAY"]
                            )
                            Game._last_turn(game_name).time_left = (
                                int(numbers_in_day[-1])
                                if len(numbers_in_day) > 0
                                else 0
                            )
                        ...
                        matching_indices = [
                            ...
                            if item[:compare_length].lower()
                            == temp_string[:compare_length].lower()
                        ]
                        if len(matching_indices) == 0:
                            ...
                            item_index = 0
                            Game._last_turn(game_name).stats[schema_name].insert(
                                item_index, temp_string
                            )
                        ...
                        closing_match = re.search(
                            schema_delimiter,
                            Game._last_turn(game_name).stats[schema_name][item_index],
                        )
                        if closing_match:
                            Game._last_turn(game_name).stats[schema_name][
                                item_index
                            ] = (
                                Game._last_turn(game_name)
                                .stats[schema_name][item_index][: closing_match.start()]
                                .strip(". \n")
                            )
                            ...
                            if schema_name == "RELATIONSHIP":
                                ...
                                if numbers:
                                    ...
                                    if last_number == 0:
                                        del Game._last_turn(game_name).stats[
                                            schema_name
                                        ][item_index]
                ...
                if count_tokens:
                    Game._(game_name).llm_model.num_tokens_from_text(
                        model, Game._last_raw(game_name)[1]
                    )
            ...
            Game._last_display(game_name)[
                0
            ] = f"Try again! Encountered an exception: {e}"
            Game._last_display(game_name)[1] = None
