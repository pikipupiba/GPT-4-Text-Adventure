...
if opening_match:
    ...
    Game._last_display(game_name)[1] = Game._last_display(game_name)[1][
        : opening_match.start()
    ]
...
if schema_name == "ITEM":
    if item_index is None:
        ...
        matching_indices = [
            ...
            if item.startswith(temp_string)
        ]
...
closing_match = re.search(
    schema_delimiter, Game._last_turn(game_name).stats[schema_name]
)
if closing_match:
    items_array = None
    item_index = None
    temp_string = ""
    Game._last_turn(game_name).stats[schema_name] = Game._last_turn(game_name).stats[
        schema_name
    ][: closing_match.start()]
