...
class ChatMessage:
    def __init__(self, role: RoleType, content: Union[str, List[str], Dict[DisplayType, str]], tokens: LilToken = None):
        self.role = role

        if role == RoleType.SYSTEM:
            self.content = {
                DisplayType.DISPLAY: None,
                DisplayType.CONTEXT: None,
                DisplayType.SUMMARY: None,
                DisplayType.SYSTEM: content,
            }
        if isinstance(content, str):
            self.content = {
                DisplayType.DISPLAY: content,
                DisplayType.CONTEXT: content,
                DisplayType.SUMMARY: None,
                DisplayType.SYSTEM: None,
            }
        elif isinstance(content, list):
            self.content = {
                DisplayType.DISPLAY: content[0],
                DisplayType.CONTEXT: content[1] if len(content) > 1 else content[0],
                DisplayType.SUMMARY: content[2] if len(content) > 2 else None,
                DisplayType.SYSTEM: None,
            }
        elif isinstance(content, dict):
            self.content = {
                DisplayType.DISPLAY: content.get(DisplayType.DISPLAY, None),
                DisplayType.CONTEXT: content.get(DisplayType.CONTEXT, None),
                DisplayType.SUMMARY: content.get(DisplayType.SUMMARY, None),
                DisplayType.SYSTEM: None,
            }
        else:
            raise TypeError(f"Invalid content type: {type(content)}")
        
        if self.role == RoleType.ASSISTANT:
            self.tokens = tokens
            self.combat = []
            self.stats = {}
        else:
    ...
    def stream_response(self, other: List[str]):
        self.content[0] += other[0] if other[0] else ""
        self.content[1] += other[1] if other[1] else ""
        self.content[2] += other[2] if other[2] else ""

    @property
    def display(self) -> Optional[str]:
        return self.content.get(DisplayType.DISPLAY, None)
    @display.setter
    def display(self, value: str) -> None:
        self.content[DisplayType.DISPLAY] = value
    @property
    def context(self) -> Optional[str]:
        return self.content.get(DisplayType.CONTEXT, None)
    @context.setter
    def context(self, value: str) -> None:
        self.content[DisplayType.CONTEXT] = value
    @property
    def summary(self) -> Optional[str]:
        return self.content.get(DisplayType.SUMMARY, None)
    @summary.setter
    def summary(self, value: str) -> None:
        self.content[DisplayType.SUMMARY] = value
        

    def __getitem__(self, key: Union[int, DisplayType]) -> str:
        if isinstance(key, int):
            try:
                key = DisplayType(key)  # Convert index to corresponding DisplayType
            except ValueError:
                raise KeyError(f"Invalid DisplayType index: {key}")
    ...
    def __repr__(self):
        return f"<ChatMessage from {self.role} contains {[dt for dt, content in self.content.items() if content is not None]}>"
    def __str__(self):
        filtered_content = {key: content for key, content in self.content.items() if content is not None}
        return f"Message from {self.role}: {filtered_content}"
